...
class AsignacionInstructController extends Controller
{
	/**
	...
	public function actionCreate()
	{
		...
		if(isset($_POST['AsignacionInstructor']))
		{
			...
			//$actividad->cantidad_clientes++;
			//aqui identifico si es personal trainer o instructor, tengo que hacerlo
			//$actividad= Actividad::model()->findByAttributes(array('attributes'=>array($this->nombre_actividad),'condition'=>'nombre=:nombre', 
														  //'params'=>array(':nombre'=>$model->nombre_actividad)));

			$actividad=Actividad::model()->findByAttributes(array('nombre'=>$model->nombre_actividad));
			if($model->save())
			{
				$actividad->rut_instructor = $model->rut_instructor;
				$actividad->cantidad_clientes++;
				if($actividad->save())
